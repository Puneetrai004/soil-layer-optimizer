...
def calculate_pressure_profile(layers, gwt_depth):
    ...
    gwt_from_top = total_height - gwt_depth if gwt_depth is not None else None
    ...
    for layer in layers:
        ...
        if gwt_from_top is None:
            gamma_eff = layer.gamma
        else:
            gamma_eff = layer.gamma if bottom <= gwt_from_top else layer.gamma - gamma_w
...
with st.expander("📘 Theory - Rankine Earth Pressure & Optimization", expanded=True):
    ...
    <h4>📌 Rankine's Active Earth Pressure Theory</h4>
    Rankine's theory assumes a uniform soil mass and ignores friction between the wall and soil. The key principles include:

    <ul>
        <li>Soil is <i>homogeneous and isotropic</i>.</li>
        <li>Soil mass is <i>semi-infinite</i>.</li>
        <li>The failure surface follows a <i>plane rupture</i>.</li>
        <li>The retaining wall is <i>smooth and vertical</i>.</li>
        <li>No wall friction is considered.</li>
        <li>Soil obeys <i>Mohr-Coulomb failure criterion</i>.</li>
        <li>The pressure acts parallel to the ground surface.</li>
    </ul>

    <b>Formula for Active Earth Pressure Coefficient:</b><br>
    \[ K_a = \frac{1 - \sin(\phi)}{1 + \sin(\phi)} \]<br>
    
    <b>Active Earth Pressure at Depth h:</b><br>
    \[ P_a = K_a \cdot \gamma \cdot h \]<br>

    <b>Total Active Force on Wall of Height H:</b><br>
    \[ F_a = \frac{1}{2} \cdot K_a \cdot \gamma \cdot H^2 \]<br>
    
    This force acts at a height of \( H/3 \) from the base of the wall.

    <p>When groundwater is present, the effective unit weight \( \gamma' \) is reduced below the groundwater table by subtracting \( \gamma_w = 9.81 \text{ kN/m}^3 \).</p>
    </div>
...
uploaded_file = st.file_uploader("📄 Upload CSV File", type="csv")
gwt_depth = st.slider("🌊 Groundwater Table Depth from Bottom (m):", min_value=0.0, max_value=50.0, value=0.0, step=0.5)
    ...
    try:
        ...
        if not all(col in df.columns for col in required_columns):
            ...
            for ax, set_layers, title in zip(axs, [layers, optimized_layers], ["Original", "Optimized"]):
                ...
                if gwt_depth is not None:
                    total_height = sum(layer.thickness for layer in set_layers)
                    gwt_from_top = total_height - gwt_depth
                    ax.axhline(y=gwt_from_top, color='blue', linestyle='--', label='GWT')
